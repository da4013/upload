	    	...
	    	<c:when test="${useCommunity == 'true'}">
	    		<td class="lt_text" nowrap>
			    	...
			    	<form name="item" method="post" action="<c:url value='/cop/com/selectBBSUseInf.do'/>"  target="_blank">
			            ...
			            <span class="link"><input type="submit" value="<c:out value="${result.bbsName}"/>" onclick="fn_egov_select_bbsUseInf('<c:out value="${result.bbsId}"/>','<c:out value="${result.trgetId}"/>'); return false;"></span>
					</form>
					-->
